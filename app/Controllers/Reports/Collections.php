...
use App\Models\Reports\CollectionReport_Model;
...
{
    /**
     * @var CollectionReport_Model
     */
    private CollectionReport_Model $m;
    public function __construct()
    {
        parent::__construct();
        $this->ValidateUser('Rep');
        $this->m=new CollectionReport_Model($this->session->get('hosID'));
    }
    ...
    public function Get(string $decCons, string $date)
    {
        $decDate=base64_decode($date);
        $cons=\base64_decode($decCons);

        $data=[
            'LabData'=>$this->m->GetCollection($cons, $decDate),
            'DueData'=>$this->m->GetDueCollected($decDate)
        ];

        $data['Date']=(object)['start'=>\DateTime::createFromFormat('Y-m-d', $decDate)->format('d/m/Y')];
        $data['showDue']=$cons==='All';

        $this->response->setHeader('Content-Encoding', 'br');

        if (!is_null($this->request->getGet('base'))) {
            $this->response->setContentType('text/plain');
            echo brotli_compress(base64_encode(view('Reports/CollectionReport', $data)));
        } else {
            $this->response->setContentType('application/pdf');
            echo \brotli_compress(view('Reports/CollectionReport', $data));
        }
    }
